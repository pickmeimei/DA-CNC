...
def main(args):
    ...
    adj_t, feature_t, label_t, idx_tot_t = load_data(dataset=args.target_dataset + '.mat', device=device,
                                                     seed=args.seed, is_blog=args.is_blog)
    #删除边实验，稀疏度分析
    # adj_s, feature_s, label_s, idx_tot_s, diff_s = load_data_diff_delete(dataset=args.source_dataset_del + '.mat',
    #                                                                      device=device,
    #                                                                      seed=args.seed, is_blog=args.is_blog)
    # adj_t, feature_t, label_t, idx_tot_t, diff_t = load_data_diff_delete(dataset=args.target_dataset_del + '.mat',
    #                                                                      device=device,
    #                                                                      seed=args.seed, is_blog=args.is_blog)
    n_samples = args.n_samples.split(',')
    output_dims = args.output_dims.split(',')
    emb_soufilename = args.source_dataset
    emb_tarfilename = args.target_dataset
    emb_model = GraphSAGE(**{
    ...
    for epoch in range(args.epochs):
        ...
        for iter in range(num_batch):
            ...
            logits_s = dgi_model(neg_source_feats, source_features, target_features)
        ...
        if (micro_f1_t + macro_f1_t) > (best_micro_f1 + best_macro_f1):
            ...
            best_epoch = epoch
            scipy.io.savemat('./' + emb_soufilename + '-' +emb_tarfilename + '_AdaGIn_emb.mat',
                             {'rep_S': embs_whole_s, 'rep_T': embs_whole_t, 'label_S': targets_whole_s,
                              'label_T': targets_whole_t})
