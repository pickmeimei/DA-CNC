...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for iter in range(num_batch):
            ...
            # logits_s = dgi_model(neg_source_feats, source_features, target_features)#es和Et正对，扰乱es负对
            # logits_t = dgi_model(neg_target_feats, target_features, source_features)

            # logits_s = dgi_model(neg_source_feats, source_features, diff_source_features)#es和Es(diff)正对，扰乱es负对
            # logits_t = dgi_model(neg_target_feats, target_features, diff_target_features)
            #跨域对比,用的是原始表示
            ...
            # features_diff = torch.cat((diff_source_features, diff_target_features), 0)
            # outputs_diff = cly_model(features_diff)
            # softmax_output_diff = nn.Softmax(dim=1)(outputs_diff)
            # domain_diff = args.cdan_param * loss_func.CDAN([features_diff, softmax_output_diff], disc_model, None, grl_lambda,
            #                                               None, device=device)

            loss = cly_loss_s + domain_loss + dgi_loss_cross
            # loss = cly_loss_s + 0.1*dgi_loss + domain_loss + dgi_loss_cross
            cly_optim.zero_grad()
            loss.backward()
            cly_optim.step()
