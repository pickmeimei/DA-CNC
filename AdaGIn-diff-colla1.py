...
###和扩散表示进行对比（1.原图和扩散图相当于两个视图，从这两个视图中学习到的表示之间的一致性最大化（最大似然估计），缓解稀疏）
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for iter in range(num_batch):
            ...
            pos_scores1 = mi_model(target_features, diff_target_features)  # 正样本的互信息得分
            neg_scores1 = mi_model(source_features, neg_target_feats)  # 负样本的互信息得分
            ...
            mi_loss1 = args.mi_param * (
                        -torch.mean(torch.log(torch.sigmoid(pos_scores)) + torch.mean(torch.log(1 - torch.sigmoid(neg_scores)))))
            mi_loss2 = args.mi_param * (
                -torch.mean(
                    torch.log(torch.sigmoid(pos_scores1)) + torch.mean(torch.log(1 - torch.sigmoid(neg_scores1)))))
            mi_loss=mi_loss1+mi_loss2
            # 跨域对比loss
