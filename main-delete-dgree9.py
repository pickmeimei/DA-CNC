...
def delete_edges(adj_matrix, ratio):
    ...
    # 筛选出度超过9的节点
    high_degree_nodes = [i for i in range(n) if degrees[i] > 9]
    # 获取所有的边，每条边用一个元组表示，包含两个节点的索引
    edges = [(i, j) for i in high_degree_nodes for j in high_degree_nodes if i < j and adj_matrix[i][j] == 1]
...
for network in netlist:
    ...
    # print(np.sum(X))
    scipy.io.savemat('./Data_delete_degree/' + network+'_degree_'+str(ratio)+'.mat', {'attrb': X, 'group': Y,'network': A},do_compression=True)
